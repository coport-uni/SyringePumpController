...
class TestStatusByte:
    def test_ready_no_error(self) -> None:
        s = Pump.StatusByte.decode(0x40)
        assert s.busy is False
        assert s.error is Pump.ErrorCode.OK
        assert s.is_ok

    def test_busy_no_error(self) -> None:
        s = Pump.StatusByte.decode(0x60)
        assert s.busy is True
        assert s.error is Pump.ErrorCode.OK

    def test_ready_not_initialized(self) -> None:
        s = Pump.StatusByte.decode(0x47)
        assert s.busy is False
        assert s.error is Pump.ErrorCode.NOT_INITIALIZED
        assert not s.is_ok

    def test_busy_plunger_overload(self) -> None:
        s = Pump.StatusByte.decode(0x69)
        assert s.busy is True
        assert s.error is Pump.ErrorCode.PLUNGER_OVERLOAD

    def test_unknown_error_code_maps_to_unknown(self) -> None:
        s = Pump.StatusByte.decode(0x45)
        assert s.error is Pump.ErrorCode.UNKNOWN

    @pytest.mark.parametrize("bad", [0x00, 0x10, 0x80, 0xFF])
    def test_missing_fixed_frame_bit_raises(self, bad: int) -> None:
        with pytest.raises(Pump.ProtocolError, match="fixed bit-6"):
            Pump.StatusByte.decode(bad)


class TestParseReply:
    def test_status_only(self) -> None:
        reply = Pump.parse_reply(_frame(0x40))
        assert reply.status.error is Pump.ErrorCode.OK
        assert reply.data == b""

    def test_with_data(self) -> None:
        reply = Pump.parse_reply(_frame(0x40, b"V1.4"))
        assert reply.data == b"V1.4"
        assert reply.status.is_ok

    def test_data_may_contain_slash_zero(self) -> None:
        reply = Pump.parse_reply(_frame(0x40, b"/0/0"))
    ...
    def test_malformed_raises(self, frame: bytes, match: str) -> None:
        with pytest.raises(Pump.ProtocolError, match=match):
            Pump.parse_reply(frame)
...
class TestLeadingGarbageTolerance:
    ...
    chip, not from the pump. The slicing logic in `Pump._send_and_receive` strips
    bytes before the leading '/'; `Pump.parse_reply` stays strict and rejects malformed
    ...
    def test_buffer_slicing_strips_leading_garbage(self) -> None:
        ...
        reply = Pump.parse_reply(cleaned)
